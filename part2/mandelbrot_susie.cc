...
main(int argc, char* argv[]) {
 ...
 int lheight = ceil((float)height/np); 
  //int lheight =height/np; 

  if(rank ==0)
  {
  // printf ("msgbuff allocated by rank %d \n", rank);
   msgbuf = (float *)malloc (lheight*width *np* sizeof (float));
   //printf ("msgbuff allocated by rank %d \n", rank);
  }
...
*/
 ...
 // for (int i = 0; i < height; ++i) {
  ...
  for(int m=0;m<np;m++)
  {
  for (int i = m; i < height;) {   
    for (int j = 0; j < width; ++j) {
      printf("image j,p = (%d,%d)  msgbuf(i,j) = (%d,%d)\n",j,p,i,j);
      img_view(j, p) = render(msgbuf[i*width+j]);
    }
    p++; 
    i= i+np;
   }
...
*/
    ...
    for (int g = 0; g < height; ++g) {
      for (int h = 0; h < width; ++h) {
        mp = (g % np) * MAX_BUFLEN; 
        img_view(h, g) = render(msgbuf[mp + (mrow * width) + h]);
      }
      mrow = g / np; 
    }
